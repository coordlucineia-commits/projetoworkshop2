...
/** Diferença entre entrada e saída (mesmo dia); retorna texto tipo `8h` ou `7h30`. */
function formatCargaHoraria(entrada: string, saida: string): string {
  const e = parseMinutes(entrada);
  const s = parseMinutes(saida);
  let diff = s - e;
...
function waUrl(telefone: string | null): string | null {
...
export function ProfessorPublicPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data } = await getSupabase().from("professores").select("*").eq("id", id).maybeSingle();
      const row = (data ?? null) as Prof | null;
      setP(row);
      setNotFound(!row?.id);
      setLoading(false);
    }
  ...
  const horario = useMemo(() => parseHorario(p?.horario_trabalho ?? null), [p?.horario_trabalho]);
  ...
  const linhasHorario = useMemo(() => {
    ...
    const diasOrdenados = [...horario.dias_semana].filter((d) => d >= 1 && d <= 7).sort((a, b) => a - b);
    return diasOrdenados.map((d) => ({
      dia: DIA_EXTENSO[d] ?? `Dia ${String(d)}`,
      entrada: ent,
      saida: sai,
      carga,
    }));
  ...
  const especialidades = useMemo(() => (p ? parseEspecialidades(p.especialidades) : []), [p]);
  ...
  const whatsappHref = p ? waUrl(p.telefone) : null;
  ...
  const pageBody =
    loading ? (
      ...
      <div className="flex flex-col items-center justify-center py-16 px-6 text-center rounded-2xl" style={{ background: "#0D0D0D", border: "1px solid #303030" }}>
        <p style={{ color: "#A8A8A8" }}>Professor não encontrado.</p>
        <button
          type="button"
      ...
      <div className="w-full min-w-0">
        ...
        {/* Card cabeçalho + grade de informações (mesmo bloco que perfil do aluno) */}
        <div
        ...
        >
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between mb-6">
            <div className="flex items-start gap-4 min-w-0">
              <AlunoAvatar nome={p.nome} src={p.foto} size={64} />
              ...
              <div className="min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <span
                    ...
                    style={{
                      background: p.ativo ? "rgba(34, 197, 94, 0.15)" : "rgba(107, 107, 107, 0.2)",
            ...
            <div className="flex items-center gap-2 shrink-0 self-start sm:self-auto">
              ...
              <button
                type="button"
                onClick={() => navigate(`/professores/editar/${p.id}`)}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                <BookOpen size={14} style={{ color: "#6B6B6B" }} />
                <span className="text-[11px] uppercase tracking-widest" style={{ color: "#6B6B6B" }}>
                  Aulas ministradas
                </span>
              </div>
              <p className="text-sm leading-relaxed" style={{ color: "#AAAAAA" }}>
                {aulasMinistradasTexto}
              </p>
            </div>
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                <Award size={14} style={{ color: "#6B6B6B" }} />
                <span className="text-[11px] uppercase tracking-widest" style={{ color: "#6B6B6B" }}>
                  Especialidades
                </span>
              </div>
            ...
            >
              ...
              {linhasHorario.map((row, idx, arr) => (
                <div
                  ...
                  style={{
                    borderBottom: idx < arr.length - 1 ? "1px solid #222222" : "none",
                    color: "#FFFFFF",
                  }}
                >
                  <span className="font-medium">{row.dia}</span>
                  <span className="md:text-center text-[#AAAAAA]">
                    <span className="md:hidden" style={{ color: "#6B6B6B" }}>
                      Entrada:{" "}
                    </span>
                    {row.entrada}
                  </span>
                  ...
                  <span className="md:text-center font-mono text-[#00F9E4]">{row.carga}</span>
                </div>
              ))}
            </div>
          )}
        ...
        >
          <div className="min-w-0">
            ...
            <p className="hidden md:block font-mono text-sm uppercase tracking-widest" style={{ color: "#606060" }}>
              Perfil público na LuTe Academy
            </p>
          </div>
